...
Flow:
  ...
  4. Send to Claude: Write an elite brief
...
def run_intelligence_cycle():
    ...
    if not elite:
        ...
        return
    logger.info("Step 3: Calling Claude for synthesis...")
    brief = generate_brief_with_claude(elite)
    if not brief:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO briefs (content, item_count) VALUES (?, ?)",
        (brief, len(elite))
    )
    conn.commit()
    conn.close()
...
def generate_brief_with_claude(elite_items: List[Dict]) -> str:
    client = Anthropic()
    items_text = "\n\n".join([
        f"ITEM {i+1} [Score: {item['composite_score']}]\nTitle: {item['title']}\nSource: {item['source']}\nSummary: {item['summary']}\nURL: {item['url']}"
        for i, item in enumerate(elite_items)
    ])
    prompt = f"""You are an elite intelligence analyst. Write a concise, actionable brief for a sophisticated decision-maker.

Top intelligence signals:

{items_text}

Your brief should be 250-350 words, scannable, with emoji for navigation, and decision triggers.
